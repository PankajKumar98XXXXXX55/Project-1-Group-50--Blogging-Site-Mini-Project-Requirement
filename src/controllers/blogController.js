...
const updateBlog = async function (req, res) {
  try {
    ...
    let update = await blogModel.findByIdAndUpdate({ _id: blogId }, { $push: { category: category, subcategory: subcategory, tags: tags }, title: title, body: body, }, { new: true });

    if (req.body.published == true && Details.published == false) {
      let finalUpdate = await blogModel.findByIdAndUpdate({ _id: blogId }, { published: true, publishedAt: moment().format(), }, { new: true });
      update = finalUpdate;
    }
...
const deleteQuery = async function (req, res) {
  try {
    ...
    let obj = await blogModel.find({$and: [anyData, {author_Id: decode.author_Id}]});
